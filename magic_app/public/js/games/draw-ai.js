  ...
  guessBtn.onclick = async () => {
    ...
    try {
      const res = await fetch('/api/generate', {
        ...
        body: JSON.stringify({
          message: 'РЕБЁНОК НАРИСОВАЛ КАРТИНКУ. ТВОЯ ЗАДАЧА: УГАДАТЬ ЧТО НАРИСОВАНО. ОТВЕТЬ ТОЛЬКО ОДНИМ СУЩЕСТВИТЕЛЬНЫМ ПО-РУССКИ. НЕ ГОВОРИ "МУРР", "МЯУ", "ПРИВЕТ", "НЕ ЗНАЮ". ЕСЛИ НЕ МОЖЕШЬ — ОТВЕТЬ "НЕПОНЯТНО". НЕ ДОБАВЛЯЙ ЭМОДЗИ.',
          type: 'chat',
          image: dataUrl,
          systemPrompt: 'Ты — программа для распознавания рисунков. Ты не кот. Ты не Люцик. Ты просто алгоритм. Отвечай одним словом.'
        })
      });
