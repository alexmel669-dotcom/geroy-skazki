...
Отвечай тёплым тоном. Пиши на русском.`;
      ...
      if (!deepseekKey) {
        return res.status(200).json({ 
          advice: '🌙 Обнимите ребёнка и скажите: "Я рядом, мы справимся вместе". Почитайте сказку про смелого героя.',
          fear 
        });
      }
      ...
      if (!response.ok) {
        return res.status(200).json({ 
          advice: '🌙 Попробуйте обнять ребёнка и сказать: "Я рядом, мы справимся вместе". Почитайте сказку перед сном.',
          fear 
        });
      }
    ...
    // ========== GET STATS (ADMIN) — через /api/index?action=get-stats или /api/index/get-stats ==========
    if (url.includes('get-stats') || query.action === 'get-stats') {
      ...
      if (!process.env.POSTGRES_URL) {
        return res.status(200).json({
          ...
          topFears: [],
          recentUsers: [],
          message: 'База данных не подключена. Статистика недоступна.'
        });
      }
      ...
      try {
        const [usersResult, storiesResult, fearsResult, todayResult, topFearsResult, recentResult] = await Promise.all([
          ...
          pool.query('SELECT email, created_at FROM users ORDER BY created_at DESC LIMIT 20')
        ]);
